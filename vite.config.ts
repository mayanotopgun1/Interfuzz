...
async function handleGenerateSeeds(req: any, res: any) {
  if (req.method !== 'POST') {
    res.statusCode = 405
  ...
  req.on('end', async () => {
    try {
      ...
      tsNode.stderr.on('data', (d: any) => { 
        stderr += d.toString()
        console.error(`[TypeScript stderr] ${d.toString()}`)
      })

      tsNode.on('close', async (code: number) => {
        if (code !== 0) {
        ...
        try {
          ...
          if (m) {
            ...
            const dirs = files.filter((f: any) => f.isDirectory() && f.name.startsWith('generated_tests_')).map((d: any) => d.name)
            if (dirs.length > 0) {
